...
<head>
    ...
    <title>EktaMart | Confirm Password</title>
...
<body>
        ...
        <div class="w-full max-w-md animate-fade-up" id="main-card">
            ...
            <div class="text-center mb-8">
                <div class="inline-flex items-center justify-center bg-gradient-to-tr from-purple-500/20 to-indigo-500/20 backdrop-blur-md p-4 rounded-3xl shadow-2xl border border-white/10 mb-4 transform transition-transform hover:scale-105 duration-300 cursor-pointer" id="logoContainer">
                    ...
                    <i class="fas fa-shield-hart text-3xl text-purple-300 ml-2"></i>
                </div>
                <h1 class="text-4xl font-bold bg-gradient-to-r from-white via-purple-200 to-indigo-300 bg-clip-text text-transparent tracking-tight">
                    Secure<span class="text-purple-400">Area</span>
                </h1>
                <p class="text-purple-200/60 text-sm mt-2 font-light">Verify your identity</p>
            </div>

            <!-- Confirm Password Card -->
            <div class="glass-premium p-6 md:p-8" id="confirmCard">
                ...
                <div class="text-center mb-5">
                    <h2 class="text-2xl md:text-3xl font-bold text-white">Confirm Access 🔒</h2>
                </div>

                <!-- Info Message -->
                <div class="info-message">
                    <i class="fas fa-info-circle"></i>
                    <p>This is a secure area of the application. Please confirm your password before continuing to access sensitive account features.</p>
                </div>
                ...
                <form method="POST" action="{{ route('password.confirm') }}" id="confirmForm" novalidate>
                    ...
                    <div class="input-wrapper">
                        ...
                        <label for="password" class="floating-label">
                            <i class="fas fa-lock text-xs"></i> Your Password
                        </label>
                    ...
                    <div class="mb-4 text-xs text-purple-300/50 flex items-center gap-2">
                        <i class="fas fa-info-circle"></i>
                    ...
                    <button type="submit" id="submitBtn" class="btn-premium w-full text-white font-semibold flex items-center justify-center gap-2">
                        ...
                        <i class="fas fa-arrow-right-to-bracket text-xs opacity-80"></i>
                    </button>
                </form>
                ...
                <div class="relative my-6">
                    ...
                    <div class="relative flex justify-center text-xs">
                        <span class="bg-[rgba(20,20,35,0.8)] px-4 text-purple-300/70 backdrop-blur-sm rounded-full">need help?</span>
                    </div>
                </div>

                <!-- Back to Dashboard / Home Link -->
                <p class="text-center">
                    <a href="{{ url('/') }}" class="text-purple-300 font-semibold hover:text-white transition link-premium inline-flex items-center gap-2">
            ...
            <div class="text-center text-white/40 text-xs mt-6 flex justify-center gap-4 flex-wrap">
                <span class="flex items-center gap-1 hover:text-white/60 transition"><i class="fas fa-fingerprint"></i> Biometric Ready</span>
                <span class="flex items-center gap-1 hover:text-white/60 transition"><i class="fas fa-shield-virus"></i> Advanced Protection</span>
                <span class="flex items-center gap-1 hover:text-white/60 transition"><i class="fas fa-clock"></i> Session Timeout</span>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        gsap.from("#confirmCard", { opacity: 0, scale: 0.95, duration: 0.6, delay: 0.2, ease: "power2.out" });
        ...
        gsap.from(".secure-badge", { opacity: 0, y: -20, duration: 0.4, delay: 0.4 });
        ...
        function validatePassword() {
            ...
            if (password.length < 6) {
                ...
                passwordError.innerHTML = '<span class="text-emerald-400 flex items-center gap-1"><i class="fas fa-check-circle text-xs"></i> Password entered</span>';
                return true;
            }
        }
        ...
        confirmForm.addEventListener('submit', function(e) {
            ...
            submitBtn.innerHTML = '<div class="spinner"></div><span class="ml-2">Verifying...</span>';
        });
        ...
        // Trigger validation on page load if password exists (from autofill)
        ...
        // Console Greeting
        console.log("%c✨ EktaMart Secure Area | Password Confirmation Required ✨", "color: #a855f7; font-size: 16px; font-weight: bold;");
        console.log("%c⚡ Security Features: Password validation • Toggle visibility • 3D tilt • Session protection", "color: #6366f1; font-size: 12px;");
        
        // Warn user before leaving if form is partially filled (optional security)
        let formDirty = false;
        passwordInput.addEventListener('input', () => { formDirty = true; });
        
        window.addEventListener('beforeunload', (e) => {
            if (formDirty && passwordInput.value && !submitBtn.classList.contains('btn-loading')) {
                e.preventDefault();
                e.returnValue = '';
            }
        });
