    ...
    <div class="relative z-10 flex items-center justify-center min-h-screen p-5">
        <div class="w-full max-w-md animate-fade-up" id="main-card">
            ...
            <div class="text-center mb-8">
                ...
                <p class="text-purple-200/60 text-sm mt-2 font-light">Create a new secure password</p>
            </div>

            <!-- Reset Password Card -->
            <div class="glass-premium p-6 md:p-8" id="resetCard">
                <div class="text-center mb-5">
                    <h2 class="text-2xl md:text-3xl font-bold text-white">Set New Password 🔐</h2>
                    <p class="text-purple-200/60 text-sm mt-1">Choose a strong password for your account</p>
                </div>
                ...
                <form method="POST" action="{{ route('password.store') }}" id="resetForm" novalidate>
                    ...
                    <!-- Email Field (hidden but visible for confirmation) -->
                    <div class="input-wrapper">
                        <input type="email" 
                               ...
                               class="premium-input"
                               readonly
                               style="background: rgba(10, 10, 25, 0.4); cursor: not-allowed;">
                    ...
                    <div class="input-wrapper">
                        ...
                        <label for="password" class="floating-label">
                            <i class="fas fa-lock text-xs"></i> New Password
                        </label>
                    ...
                    <div class="requirements-box mb-4">
                        <p class="text-xs text-purple-300/70 mb-2 flex items-center gap-1">
                            <i class="fas fa-shield-alt"></i> Password requirements:
                        </p>
                        <div class="requirement-item" id="reqLength">
                            <i class="fas fa-circle"></i> <span>At least 6 characters</span>
                        </div>
                        <div class="requirement-item" id="reqUppercase">
                            <i class="fas fa-circle"></i> <span>At least 1 uppercase letter</span>
                        </div>
                        <div class="requirement-item" id="reqNumber">
                            <i class="fas fa-circle"></i> <span>At least 1 number</span>
                        </div>
                        <div class="requirement-item" id="reqSpecial">
                            <i class="fas fa-circle"></i> <span>At least 1 special character (!@#$%^&*)</span>
                        </div>
                    </div>
                    ...
                    <div class="input-wrapper">
                        ...
                        <label for="password_confirmation" class="floating-label">
                            <i class="fas fa-check-circle text-xs"></i> Confirm New Password
                        </label>
                    ...
                    <!-- Submit Button -->
                    <button type="submit" id="submitBtn" class="btn-premium w-full text-white font-semibold flex items-center justify-center gap-2 mt-4">
                        <i class="fas fa-sync-alt"></i>
                        <span>Reset Password</span>
                        <i class="fas fa-check-circle text-xs opacity-80"></i>
                    </button>
                </form>
                ...
                <div class="relative my-6">
                    ...
                    <div class="relative flex justify-center text-xs">
                        <span class="bg-[rgba(20,20,35,0.8)] px-4 text-purple-300/70 backdrop-blur-sm rounded-full">remembered your password?</span>
                    </div>
                </div>
            ...
            <div class="text-center text-white/40 text-xs mt-6 flex justify-center gap-4 flex-wrap">
                <span class="flex items-center gap-1 hover:text-white/60 transition"><i class="fas fa-shield-alt"></i> Secure Reset</span>
                <span class="flex items-center gap-1 hover:text-white/60 transition"><i class="fas fa-clock"></i> Link Expires in 60 mins</span>
                <span class="flex items-center gap-1 hover:text-white/60 transition"><i class="fas fa-database"></i> End-to-End Encrypted</span>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function updateRequirements() {
            ...
            if (password.length >= 6) {
                ...
                reqLength.innerHTML = '<i class="fas fa-check-circle text-emerald-400"></i> <span>At least 6 characters</span>';
            } else {
            ...
            if (/[A-Z]/.test(password)) {
                ...
                reqUppercase.innerHTML = '<i class="fas fa-check-circle text-emerald-400"></i> <span>At least 1 uppercase letter</span>';
            } else {
                reqUppercase.classList.add('requirement-unmet');
                reqUppercase.classList.remove('requirement-met');
                reqUppercase.innerHTML = '<i class="fas fa-circle"></i> <span>At least 1 uppercase letter</span>';
            }
            
            if (/[0-9]/.test(password)) {
                reqNumber.classList.add('requirement-met');
                reqNumber.classList.remove('requirement-unmet');
                reqNumber.innerHTML = '<i class="fas fa-check-circle text-emerald-400"></i> <span>At least 1 number</span>';
            } else {
                reqNumber.classList.add('requirement-unmet');
                reqNumber.classList.remove('requirement-met');
                reqNumber.innerHTML = '<i class="fas fa-circle"></i> <span>At least 1 number</span>';
            }
            
            if (/[^A-Za-z0-9]/.test(password)) {
                reqSpecial.classList.add('requirement-met');
                reqSpecial.classList.remove('requirement-unmet');
                reqSpecial.innerHTML = '<i class="fas fa-check-circle text-emerald-400"></i> <span>At least 1 special character (!@#$%^&*)</span>';
            } else {
                reqSpecial.classList.add('requirement-unmet');
                reqSpecial.classList.remove('requirement-met');
                reqSpecial.innerHTML = '<i class="fas fa-circle"></i> <span>At least 1 special character (!@#$%^&*)</span>';
            }
        }
        ...
        function validatePassword() {
            ...
            if (!isValid) {
                ...
                passwordError.innerHTML = '<span class="text-yellow-400 flex items-center gap-1"><i class="fas fa-exclamation-triangle text-xs"></i> Please meet all password requirements</span>';
                return false;
            } else {
                ...
                if (strength >= 3) {
                    ...
                    passwordError.innerHTML = '<span class="text-emerald-400 flex items-center gap-1"><i class="fas fa-check-circle text-xs"></i> Password meets requirements</span>';
                }
                return true;
            }
        }
        ...
        resetForm.addEventListener('submit', function(e) {
            ...
            submitBtn.innerHTML = '<div class="spinner"></div><span class="ml-2">Resetting password...</span>';
        });
        ...
        // Focus effect
        const allInputs = document.querySelectorAll('.premium-input');
        ...
        console.log("%c✨ EktaMart Password Reset | Secure & Strong Password Setup ✨", "color: #a855f7; font-size: 16px; font-weight: bold;");
        console.log("%c⚡ Features: Real-time validation • Password strength meter • Requirements checklist • 3D tilt", "color: #6366f1; font-size: 12px;");
    </script>
</body>
</html>